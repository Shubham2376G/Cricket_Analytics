...
def draw_foot_keypoints(frame, keypoints, keypoint_scores, court_polygon):
    ...
    foot_positions = {15: None, 16: None}  # Store foot coordinates

    for idx in foot_indices:
        if idx < len(keypoints) and idx < len(keypoint_scores):
            x, y = keypoints[idx]
            foot_positions[idx] = (int(x), int(y))  # Store coordinates
            if is_point_in_polygon((int(x), int(y)), court_polygon):
                ...
                cv2.putText(frame, str(confidence), (int(x) + 10, int(y) - 10), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    
    return foot_positions  # Return coordinates

def draw_hand_keypoints(frame, keypoints, keypoint_scores, court_polygon):
    hand_indices = [10, 8, 6, 5]  # Indices for hand keypoints
...
def detect_bowler(frame_number,frame, keypoints, keypoint_scores, court_polygon,bowler_frame_count):
    ...
    if all(idx < len(keypoints) and idx < len(keypoint_scores) for idx in hand_indices):
        ...
        if p1[1] < p2[1] and p2[1] < p3[1] and p3[1] < p4[1]:
            # Calculate the angle
            angle_elbow = calculate_angle(p1, p2, p3)
                    # Draw the angle arc on the frame
            draw_angle_arc(frame, p1, p2, p3, angle_elbow)
            
            angle_shoulder = calculate_angle(p2, p3, p4)
            draw_angle_arc(frame, p1, p2, p3, angle_shoulder)

            # Check if the angle is close to 180 degrees (straight line)
            if 160 < angle_elbow < 180 and 140 < angle_shoulder < 180 :
                ...
                draw_angle_arc(frame, p1, p2, p3,angle_elbow)
                 # Check if the angle is close to 180 degrees (straight line)
           # if 160 < angle_shoulder < 180:
                # Draw angle on frame and mark the player as bowler
               # cv2.putText(frame, 'Bowler', (int(p2[0] - 10), int(p2[1] - 25)),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.line(frame, (int(p1[0]), int(p1[1])), (int(p2[0]), int(p2[1])), (0, 0, 255), 2)
                cv2.line(frame, (int(p2[0]), int(p2[1])), (int(p3[0]), int(p3[1])), (0, 0, 255), 2)
                cv2.line(frame, (int(p3[0]), int(p3[1])), (int(p4[0]), int(p4[1])), (0, 0, 255), 2)
                                # Call draw_angle_arc to visualize the angle on the frame
                draw_angle_arc(frame,p2,p3,p4,angle_shoulder)
                if cv2.pointPolygonTest(np.array(court_polygon), p2, False) >= 0:
                    foot_positions = draw_foot_keypoints(frame, keypoints, keypoint_scores, court_polygon)
                    draw_court_region(frame, court_polygon)
                        # Get left and right foot positions
                    left_foot = foot_positions.get(15, (None, None))
                    right_foot = foot_positions.get(16, (None, None))

                    # Display frame number and foot coordinates
                    text = f"Frame: {frame_number} | Left Foot: {left_foot} | Right Foot: {right_foot}"
        ...
        if frame_index < len(data):
            ...
            for instance in instances:
                ...
                if keypoints and keypoint_scores:
                    ...
                    if detect_bowler(frame_number,frame, keypoints, keypoint_scores, court_polygon,bowler_frame_count):
...
# Example Mohit Usage
video_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\bowlers_vid\Mohit Rathee - Right arm Leg break - Trim.mp4"
json_path = r"D:\IIT_Madras_Task\mmpose\bowlerstatic_cric2\results_Mohit Rathee - Right arm Leg break - Trim.json"
output_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\MohitRathee_output_keysimu_player.avi"
output_json_path = r'D:\IIT_Madras_Task\mmpose\playershawkeye2\outputforMohitRathee_hand&foot_keypoints.json'
court_polygon = [(476, 174), (797, 174), (797, 455), (476, 455)]#Mohit
output_folder = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\MohitRathee_bowler_det"
# # Example Rakshid Dar usage
# video_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\bowlers_vid\Rasikh Dar - Right arm fast medium - Trim.mp4"
# json_path = r"D:\IIT_Madras_Task\mmpose\bowlerstatic_cric2\results_Rasikh Dar - Right arm fast medium - Trim.json"
# output_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\RasikhDar_output_keysimu_player.avi"
# output_json_path = r'D:\IIT_Madras_Task\mmpose\playershawkeye2\outputforRasikhDar_hand&foot_keypoints.json'
# output_folder = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\RasikhDar_bowler_det"
# court_polygon = [(739, 342), (1216, 342), (1216, 820), (739, 820)]#rakshidar

#example Swapnil Singh left hander
#court_polygon = [(476, 178), (794, 178), (794, 460), (476, 460)]#SwapnilSingh
# # Example SuyushSharma usage
# video_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\bowlers_vid\Suyush Sharma - Right arm Leg break - Trim.mp4"
# json_path = r"D:\IIT_Madras_Task\mmpose\bowlerstatic_cric2\results_Suyush Sharma - Right arm Leg break - Trim.json"
# output_path = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\SuyushSharma_output_keysimu_player.avi"
# output_json_path = r'D:\IIT_Madras_Task\mmpose\playershawkeye2\outputforSuyushSharma_hand&foot_keypoints.json'
# output_folder = r"D:\IIT_Madras_Task\Pavani_cricket\CRICKET DATA COLLECTION\HAWKEYE_OUTPUTS\SuyushSharma_bowler_det"
